...
- Скопировал код из example к себе и немного изенил его (путь к модулю, максимальный размер)
...
Устанавливается terraform vault через apt, хотя в офф документации написано, что можно дёрнуть docker с ним, но мне показалось так проще.

Первое знакомство как-то не впечатлило. Из-за развития микросервисов и "уплотнения" инфраструктуры, пришлось автоматизировать процесс авторизации и расширения функционала. Хранить не только пароли но и секреты. При этом авторизация выделяется как отдельный слой: OPA, SPIFFE/SPIRE, Istio. Как итог, вместо LDAP имеем  OIDC + короткоживущие токены.
...
В целом задача простая, вся работа идёт через **vault_generic_secret**. Можно читать как с ним работать из CLI из официальной документации, по крайней мере логика работы и необходимые параметры будет ясна.
